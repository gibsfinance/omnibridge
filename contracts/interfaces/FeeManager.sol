...
import { MediatorOwnableModule } from "../upgradeable_contracts/modules/MediatorOwnableModule.sol";

abstract contract FeeManager is MediatorOwnableModule {
    ...
    function calculateFee(
        ...
        if (_rewardAddressesCount() == 0) {
            return 0;
        }
        uint256 _fee = getFee(_feeType, _token);
        return (_value * _fee) / MAX_FEE;
    }
     ...
     */
    function _isValidAddress(address _addr) internal view virtual returns (bool) {
     ...
     */
    function rewardAddressesCount() external virtual view returns (uint256) {
        return _rewardAddressesCount();
    }
    function _rewardAddressesCount() internal virtual view returns(uint256);
     ...
     */
    function isRewardAddress(address _addr) external view virtual returns (bool) {
        return _isRewardAddress(_addr);
    }
    function _isRewardAddress(address _addr) internal view virtual returns(bool);
     ...
     */
    function distributeFee(address _token) external virtual;
}
